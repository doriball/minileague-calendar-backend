...
import io.doriball.modulecalendar.fixture.domain.storeEventRuleFixture
import io.doriball.modulecalendar.fixture.domain.storeFixture
...
import io.doriball.modulecalendar.store.application.service.StoreServiceV1
...
class StoreServiceV1UnitTest {
    ...
    @InjectMocks
    lateinit var sut: StoreServiceV1

    @Mock
    lateinit var storePort: StorePort

    @Mock
    lateinit var storeEventRulePort: StoreEventRulePort
    ...
    fun 매장_목록_조회_성공() {
        ...
        given(storePort.getStores(command.regionNo)).willReturn(listOf(storeFixture()))
    ...
    fun 매장_상세_조회_성공() {
        ...
        // given
        val command = ReadStoreDetailCommand("store-1")
        given(storePort.getStoreDetail(command.storeId)).willReturn(storeFixture())
        given(storeEventRulePort.getStoreEventRules(command.storeId)).willReturn(listOf(storeEventRuleFixture()))
    ...
    fun 매장_상세_조회_실패_존재하지_않음() {
        ...
        // given
        val command = ReadStoreDetailCommand("store-2")
        given(storePort.getStoreDetail(command.storeId)).willReturn(null)
